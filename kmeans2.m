function [idxbest, Cbest, sumDbest, Dbest] = kmeans2(X, k,mspe,Sspe, varargin)
...
if hadNaNs
    ...
    function cellout = loopBody(rep,S)
        ...
        D = distfun(X, C, distance, 0, rep, reps,mspe,Sspe);
        [d, idx] = min(D, [], 2);
        ...
        try % catch empty cluster errors and move on to next rep
            ...
            nonempties = find(m>0);
            D(:,nonempties) = distfun(X, C(nonempties,:), distance, iter, rep, reps,mspe,Sspe);
            d = D((idx-1)*n + (1:n)');
        ...
        function converged = batchUpdate()
            ...
            while true
                ...
                D(:,changed) = distfun(X, C(changed,:), distance, iter, rep, reps,mspe,Sspe);
                    ...
                    switch emptyact
                        ...
                        case 'singleton'
                            for i = empties
                                ...
                                m(i) = 1;
                                idx(lonely) = i;
                                D(:,i) = distfun(X, C(i,:), distance, iter, rep, reps,mspe,Sspe);
                                
                                % Update clusters from which points are taken
                                [C(from,:), m(from)] = gcentroids(X, idx, from, distance);
                                D(:,from) = distfun(X, C(from,:), distance, iter, rep, reps,mspe,Sspe);
...
function D = distfun(X, C, dist, iter,rep, reps,mspe,Sspe)
...
switch dist
    case 'sqeuclidean'
        ...
        for i = 1:nclusts
%             for h=1:n
%                 dspa = sqrt((X(h,4) - C(i,4))^2 + (X(h,5) - C(i,5))^2);
%                 if dspa>Sspe
%                     %D(h,i) = Inf;
%                     D(h,i)=10000;
%                 else
%                     dcouleur = sqrt((X(h,1) - C(i,1))^2 + (X(h,2) - C(i,2))^2 + (X(h,3) - C(i,3))^2);
%                     D(h,i) = dcouleur + kon*dspa;
%                 end;
%             end;
              dspa = (X(:,4) - C(i,4)).^2 + (X(:,5) - C(i,5)).^2;
              
                    %D(h,i)=10000;
              dcouleur = (X(:,1) - C(i,1)).^2 + (X(:,2) - C(i,2)).^2 + (X(:,3) - C(i,3)).^2;
              D(:,i) = dcouleur(:) + kon*dspa(:);
        end;
